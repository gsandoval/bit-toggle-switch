    ...
    ng.module('bitToggleSwitch').directive('bitToggleSwitch', [function () {
        ...
        return {
            ...
            template: function (element, attrs) {
                return '<div class="toggle-switch" data-ng-click="toggle()">' +
                            '<div class="on-state state" data-ng-class="{ \'on-state-visible\': status, \'on-state-hidden\': !status }">' +
                                '<div class="state-label">{{labels.on}}</div>' +
                                '<div class="indicator"></div>' +
                            '</div>' +
                            '<div class="off-state state" data-ng-class="{ \'off-state-visible\': !status, \'off-state-hidden\': status }">' +
                                '<div class="indicator"></div>' +
                                '<div class="state-label">{{labels.off}}</div>' +
                            '</div>' +
                        '</div>'
            },
            transclude: true,
            link: function (scope, element, attrs, ctrl, transcludeFn) {
                ...
                function addStylesToDOM(width) {
                    ...
                    style.type = 'text/css';
                    style.innerHTML = '.' + instanceId + ' .on-state-visible { left: 0; } ' +
                        '.' + instanceId + ' .on-state-hidden { left: -' + width + 'px; } ' +
                        '.' + instanceId + ' .off-state-visible { left: 0; } ' +
                        '.' + instanceId + ' .off-state-hidden { left: ' + width + 'px; }';
